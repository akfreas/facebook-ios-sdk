...
-(id)initWithRadius:(CGFloat)theRadius {
    ...
    if (self) {
        _radius = theRadius;
        self.frame = CGRectMake(0, 0, theRadius * 2 + 1.0f, theRadius * 2 + 1.0f);
...
-(void)drawInContext:(CGContextRef)ctx {
    ...
    if (_image != nil) {
        ...
        CGSize frameSize = CGSizeMake(imageWidth, imageHeight);
        imgRect = CGRectMake(self.bounds.size.width / 2 - frameSize.width / 2, self.bounds.size.height / 2 - frameSize.height / 2, frameSize.width, frameSize.height);
